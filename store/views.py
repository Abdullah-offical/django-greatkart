...
def store(request, category_slug=None):
    ...
    if category_slug != None:
        ...
        paginator = Paginator(products, 6) # get 6 products
...
def product_details(request, category_slug, product_slug):
    try:
        ...
        in_cart = CartItem.objects.filter(cart__cart_id=_cart_id(request), product=single_product).exists() # cart is foreing key of cart model and cart id is exixt in cart model
